...
def load_agent_list(file):
    if file is not None:
        df = pd.read_excel(file)
        # Assuming the first column contains MemberId or there's a column named 'MemberId'
        if 'MemberId' in df.columns:
            return set(df['MemberId'].astype(str).unique())
        else:
            # Assume first column is MemberId
            return set(df.iloc[:, 0].astype(str).unique())
    return set()
...
if uploaded_file is not None:
    ...
    agent_ids = load_agent_list(agent_file)
    
    # Add User Type column
    df['User Type'] = df['MemberId'].astype(str).apply(
    ...
    # Only include Debit transactions for most categories, Credit for Cash in
    ...
    col1.metric("👥 Total Agents", f"{len(agent_ids):,}" if agent_ids else "0 (No agent list uploaded)")
    col2.metric("👤 Total Users", f"{df_filtered[df_filtered['User Type'] == 'User']['MemberId'].nunique():,}")
    
    if agent_ids:
        st.success(f"✅ Agent list loaded with {len(agent_ids):,} agents")
    else:
    ...
    # Group by User Type, Value Range, and Transaction Category
    ...
    grand_total = pivot_report[['Total Transaction Count', 'Total Amount (Rs)']].sum()
    ...
    with col2:
        st.metric("📦 Total Transactions", f"{grand_total['Total Transaction Count']:,.0f}")
        st.metric("💰 Total Volume", f"Rs. {grand_total['Total Amount (Rs)']:,.2f}")
    ...
    # By Count
    category_count = df_filtered.groupby('Transaction Category')['Transaction Count'].sum().reset_index() if 'Transaction Count' in df_filtered.columns else df_filtered.groupby('Transaction Category').size().reset_index(name='Count')
    if 'Transaction Count' not in df_filtered.columns:
        category_count = df_filtered.groupby('Transaction Category').size().reset_index(name='Count')
    else:
        category_count = df_filtered.groupby('Transaction Category')['Transaction Count'].sum().reset_index()
    
    fig_count = px.pie(category_count, values='Count' if 'Count' in category_count.columns else 'Transaction Count', 
                       names='Transaction Category', title='Transactions by Category')
    ...
    merchant_services = [s for sublist in [TRANSACTION_CATEGORIES['Merchant payment'], TRANSACTION_CATEGORIES['Government payment (P2G)'], TRANSACTION_CATEGORIES['Topup']] for s in sublist]
    merchant_data = df_filtered[df_filtered['Service'].isin(merchant_services)]
    
    if len(merchant_data) > 0:
        merchant_summary = merchant_data.groupby('Service').agg({
            'Amount (Rs)': ['count', 'sum']
        }).reset_index()
        merchant_summary.columns = ['Merchant Name', 'Transaction Count', 'Total Volume (Rs)']
        merchant_summary = merchant_summary.sort_values('Total Volume (Rs)', ascending=False)
        st.dataframe(merchant_summary.head(30).style.format({
            'Total Volume (Rs)': 'Rs. {:,.2f}'
        }), use_container_width=True)
    
    # Cash-In Modes Analysis
    st.subheader("💰 Cash-In Analysis by Mode")
    cash_in_data = df_filtered[df_filtered['Transaction Category'] == 'Cash in']
    if len(cash_in_data) > 0:
        ...
        st.dataframe(cash_in_summary.head(20).style.format({
            'Total Volume (Rs)': 'Rs. {:,.2f}'
        }), use_container_width=True)
    ...
    if search_input:
        ...
        user_data = df[
            (df['MemberId'].astype(str) == search_input) | 
            (df['ContactNumber'].astype(str).str.contains(search_input, na=False)) |
            (df['Name'].astype(str).str.lower().str.contains(search_lower, na=False))
        ]
        if len(user_data) > 0:
            user_type = 'Agent' if user_data['MemberId'].astype(str).iloc[0] in agent_ids else 'User'
            st.write(f"### User Type: {user_type}")
            display_cols = ['CreatedDate', 'Service', 'Sign', 'Amount (Rs)', 'Remarks']
            available_cols = [col for col in display_cols if col in user_data.columns]
            st.dataframe(user_data[available_cols].sort_values('CreatedDate', ascending=False))
    
else:
    st.info("👈 Please upload your Transaction Excel file to get started")
    st.markdown("""
    ### 📋 Instructions:
    1. **Upload Transaction Excel File** - Your main transaction data
    2. **Upload Agent List Excel File** (Optional) - Excel file containing Agent MemberIds
       - The file should have a column named 'MemberId' OR the first column should contain MemberIds
    3. The dashboard will automatically generate the Agent vs User segmentation report
    """)
